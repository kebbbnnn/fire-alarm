...
public class FrameLayoutCompat extends ViewGroup {
  ...
  void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    mForegroundInPadding = a.getBoolean(R.styleable.FrameLayoutCompat_foregroundInsidePadding,
        true);
  ...
  /**
   * Returns a set of layout parameters with a width of {@link android.view.ViewGroup
   * .LayoutParams#MATCH_PARENT}, and a height of {@link android.view.ViewGroup
  ...
  /**
   * Applies a tint to the foreground drawable. Does not modify the current tint mode, which is
   * {@link android.graphics.PorterDuff.Mode#SRC_IN} by default. <p> Subsequent calls to {@link
   * #setForeground(android.graphics.drawable.Drawable)} will automatically mutate the drawable and
   * apply the specified tint and tint mode using {@link android.graphics.drawable
   * .Drawable#setTintList(android.content.res.ColorStateList)}.
   *
   ...
   * #setForegroundTintList(android.content.res.ColorStateList)}} to the foreground drawable. The
   * default mode is {@link android.graphics.PorterDuff.Mode#SRC_IN}.
   *
   ...
   *
   * @param curState The current state as returned from a view or the result of combining multiple
   *                 views.
  ...
  /**
   * Utility to reconcile a desired size and state, with constraints imposed by a MeasureSpec.  Will
   * take the desired size, unless a different size is imposed by the constraints.  The returned
   * value is a compound integer, with the resolved size in the {@link #MEASURED_SIZE_MASK} bits and
   * optionally the bit {@link #MEASURED_STATE_TOO_SMALL} set if the resulting size is smaller than
  ...
  /**
   * Sets whether to consider all children, or just those in the VISIBLE or INVISIBLE state, when
   * measuring. Defaults to false.
   *
   * @param measureAll true to consider children marked GONE, false otherwise. Default value is
   *                   false.
  ...
  /**
   * Determines whether all children, or just those in the VISIBLE or INVISIBLE state, are
   * considered when measuring.
   *
   * @return Whether all children are considered when measuring.
   * @deprecated This method is deprecated in favor of {@link #getMeasureAllChildren()
   * getMeasureAllChildren()}, which was renamed for consistency with {@link
